...
from resnet import ResNet20Test, ResNet20x6Test, ResNet20x7Test, ResNet20x8Test, ResNet20TestSavefile
...
# 将模型保存为txt文件
def savetxt():
    pm = torch.load('../cnn/99.68best_acc_mnist.pth')['model']

    fp = os.open("../data/parameters1.txt", os.O_RDWR|os.O_CREAT)
...
def testdata():
    ...
    _,data_loader_test = read_mnist(batchsize,data_dir)

    # 保存数据参数
    # pm=torch.load('../cnn/99.69best_acc_mnist.pth')["model"]
    ...
    model = ResNet20TestSavefile().to(device)
    print(model)

    state_dict = torch.load('../cnn/99.68best_acc_mnist.pth')
...
def testCode():
    ...
    data_loader_train,data_loader_test = read_mnist(batchsize,data_dir)
...
def testReLUx6():
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # model = ResNet20().to(device)
    model = ResNet20x6Test().to(device)
    # print(model)
...
def testReLUx7():
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # model = ResNet20().to(device)
    model = ResNet20x7Test().to(device)
    # print(model)
...
def testReLUx8():
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # model = ResNet20().to(device)
    model = ResNet20x8Test().to(device)
    # print(model)
...
if __name__ == '__main__':
    # savetxt()
    # print("testCode，这是原版的ResNet20")
    # testCode()
    # print("testReLUx6，这是ResNet20x6")
    # testReLUx6()
    print("testReLUx7，这是ResNet20x7")
    testReLUx7()
    # print("testReLUx8，这是ResNet20x8")
    # testReLUx8()
    # testdata()